...
import com.cmazxiaoma.wx.dto.LocationMessage;
import com.cmazxiaoma.wx.http.HttpAPIService;
import com.cmazxiaoma.wx.util.MessageUtil;
import com.cmazxiaoma.wx.util.WxCheckUtil;
import com.cmazxiaoma.wx.constant.WxMessage;
import com.cmazxiaoma.wx.util.WxHttpUtil;
...
public class PortalController extends BaseController {
    ...
    public void sendTextMessage() throws IOException {
        ...
        if (WxMessage.TEXT.equalsIgnoreCase(msgType)) {
        ...
        if (msgType.equals(WxMessage.EVENT)) {
            String eventType = map.get("Event");

            log.info("Event = {}", eventType);

            if (eventType.equals(WxMessage.EVENT_SUBSCRIBE)) {
                message = MessageUtil.initTextMessage(fromUserName, toUserName, MessageUtil.subscribeText());
            }

            if (eventType.equals(WxMessage.EVENT_UNSUBSCRIBE)) {
                //todo
            }

            if (eventType.equals(WxMessage.EVENT_CLICK)) {
                //todo
            }

            if (eventType.equals(WxMessage.EVENT_VIEW)) {
                //todo
            }

            //通过用户授权，获取用户地理信息
            if (eventType.equalsIgnoreCase(WxMessage.EVENT_LOCATION)) {
            ...
            //用户点击上传地理信息
            if (eventType.equalsIgnoreCase(WxMessage.EVENT_LOCATION_SELECT)) {
                //事件KEY值
                ...
                locationMessage.setMsgType(WxMessage.EVENT_LOCATION);
        ...
        if (msgType.equals(WxMessage.IMAGE)) {
            //todo
        }

        if (msgType.equals(WxMessage.VIDEO)) {
            //todo
        }

        if (msgType.equals(WxMessage.VOICE)) {
            //todo
        }
